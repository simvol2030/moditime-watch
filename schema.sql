...
-- ============================================
-- NOTE: AI columns (response_mode, model, tokens_prompt, tokens_completion, cost)
-- are added via migrateChatbotAI() in database.ts with try/catch for idempotency.
-- ALTER TABLE cannot be in schema.sql because db.exec() has no per-statement error handling.
